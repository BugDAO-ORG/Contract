...
const main = async (): Promise<any> => {
    ...
    const leaf = keccak256(ethers.utils.solidityPack(["address", "uint256", "uint256"], ["0x938E2bd645b03a4aFF49D6f764663d8746c7fcF5",
        BigNumber.from("1020000000000000000000"), 1642067999]));
    console.log(leaf.toString('hex'));
    const proof = tree.getHexProof(leaf);
    console.log(proof);
    let res = await contractAsSigner1.claimTokens(BigNumber.from("1020000000000000000000"), 1642067999, proof);
